...
for i in Chr1 Chr2 Chr3 Chr4 Chr5 ChrM ChrC; do msa_view ${i}.maf --in-format MAF --4d --features ${i}.gtf > 4dsite/${i}.4d-codons.ss ; done;
...
do 
    base=$(basename ${i} .maf)
    echo "nohup phastCons --target-coverage 0.25 --expected-length 12 --rho 0.4 --estimate-rho ${base}.rho --msa-format MAF ${base}.maf 4dsite/nonconserved-4d.${base}.mod --no-post-probs &" >> estimate-rho.sh
done
...
do      
    base=$(basename ${i} .maf)   
    echo "nohup phastCons --target-coverage 0.25 --expected-length 12 --rho 0.4 --msa-format MAF ${base}.maf --most-conserved phastCons.${base}.bed ${base}.rho.cons.mod,${base}.rho.noncons.mod > phastCons.${base}.wig &" >> con.element.sh 
done
